EditTags = React.createClass({
	...
	componentDidMount(){		
		var countChecked = function() {
			...
			if(n == 3 ){
				jQuery('input:not(:checked)').parent('.editcheck').addClass('hidden');
			}
			else if(n < 3){
				jQuery('input:not(:checked)').parent('.editcheck').removeClass('hidden');
			}
		};
		countChecked();
	...
	getTags(){
		return(
    		...
    		<div className="checkbox checkbox-success checkbox-inline editcheck">
			    <input type="checkbox" value="Clase difícil" id="checkbox1" />
			    <label htmlFor="checkbox1"><p>Clase difícil</p></label>
			  </div>
			  <div className="checkbox checkbox-success checkbox-inline editcheck">
			    <input type="checkbox" value="Buen Feedback" id="checkbox2" />
			    <label htmlFor="checkbox2"><p>Buen Feedback</p></label>
			  </div>
			  <div className="checkbox checkbox-success checkbox-inline editcheck">
			    <input type="checkbox" value="Muchas tareas" id="checkbox3" />
			    <label htmlFor="checkbox3"><p>Muchas tareas</p></label>
			  </div>
			  <div className="checkbox checkbox-success checkbox-inline editcheck">
			    <input type="checkbox" value="Prepárate para leer" id="checkbox4" />
			    <label htmlFor="checkbox4"><p>Prepárate para leer</p></label>
			  </div>
			  <div className="checkbox checkbox-success checkbox-inline editcheck">
			    <input type="checkbox" value="Controles sorpresa" id="checkbox5" />
			    <label htmlFor="checkbox5"><p>Controles sorpresa</p></label>
			  </div>
			  <div className="checkbox checkbox-success checkbox-inline editcheck">
			    <input type="checkbox" value="Participación obligatoria" id="checkbox6" />
			    <label htmlFor="checkbox6"><p>Participación obligatoria</p></label>
			  </div>
			  <div className="checkbox checkbox-success checkbox-inline editcheck">
			    <input type="checkbox" value="Estricto con la asistencia" id="checkbox7" />
			    <label htmlFor="checkbox7"><p>Estricto con la asistencia</p></label>
			  </div>
			  <div className="checkbox checkbox-success checkbox-inline editcheck">
	...
	removeTags(){
		let reviewId = this.data.review._id;
		    //professorId = this.data.review.professorId;
		new Confirmation({
			title: 'Al cambiar los tags deberás rehacer los cambios editados hasta aquí',
	...
	hasTags(){
		if(this.data.review.tags.length===0){
			return false
		}
		else{
			return true
	...
	getPrevTags(){
		let review = this.data.review.tags.join(', ');
		return review;
	},
		...
		return(
			<div>
				<div className="col-sm-4">
		  		<label htmlFor="tags"><h3>Selecciona los 3 tags que mejor describen al profesor <small>(opcional)</small></h3></label>
		  	</div>
		  	{this.hasTags() ?
		  		<div>
		  			...
		  			<div className="col-sm-2">
		  				<button type="button" className="btn btn-warning btn-sm" onClick={this.removeTags}>Cambiar</button>
		  			</div>
		  		</div>
		  		:
